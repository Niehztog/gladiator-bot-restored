/* be_ea.h — interface of be_ea.c, an original Gladiator Bot v0.96
 * translation unit (Mr. Elusive, 1999). */
#ifndef BOTLIB_BE_EA_H
#define BOTLIB_BE_EA_H

/* ea_state_t: the per-client elementary-action state this TU owns. */

/* The ACTION_* bits stored in ea_state_t.flags are deliberately NOT defined here: they
 * are part of the botlib<->game interface and belong to ../game/botlib.h, which
 * botlib_port.h pulls into every botlib TU ahead of this header.  Q3's be_ea.c takes
 * them the same way and defines only its private jump latch locally.  Worth knowing
 * while reading below: ACTION_JUMP/ACTION_MOVEUP share bit 8, and
 * ACTION_CROUCH/ACTION_MOVEDOWN share bit 16. */
